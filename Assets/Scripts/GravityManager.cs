...
public class GravityManager : MonoBehaviour
{
    ...
    public void ChangeGravity()
    {
        ...
        switch (gScale)
        {
            case 0: // x(-1.0)
                ...
                moveSpeed = M_SPEED;
                if (isAvailable/* || isAvailable*/) { gravityValueText.text = "-１.０G / 反転"; } // string.Format("{ 0,6}","-1.0G") + " / 反転";//"重力場：-1.0G / 反転";
                
                break;
            case 1: // x0.5
                gravityScale = G_SCALE * 0.5f;
                moveSpeed = M_SPEED * 0.75f;
                if (isAvailable/* || isAvailable*/) { gravityValueText.text = "０.５G / 　軽"; }
                // string.Format("{0,6}","0.5G") + " / 　軽"; //"重力場：0.5G / 軽";
                break;
            case 2: // x2.0
                gravityScale = G_SCALE * 2.0f;
                moveSpeed = M_SPEED * 1.5f;
                if (isAvailable/* || isAvailable*/) { gravityValueText.text = "２.０G / 　重"; } // string.Format("{ 0,6}", "2.0G") + " / 　重";//"重力場：2.0G / 重";
                break;
            default:
    ...
    {
        yield return new WaitForSecondsRealtime(4.5f); // 5秒遅延
        destroyGF = GameObject.FindWithTag("GravityField");
        if (destroyGF != null)
        {
